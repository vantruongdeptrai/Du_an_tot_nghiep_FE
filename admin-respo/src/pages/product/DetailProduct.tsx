...
const DetailProduct: React.FC = () => {
    ...
    const getProductById = async (id: string | undefined) => {
        ...
        try {
            ...
            const foundProduct = response.data.find((item: any) => item.id === parseInt(id));
    ...
    const getProductVariantById = async (product_id: string) => {
        try {
            ...
            const variantsData = response.data.filter((variant: any) => variant.product_id === parseInt(product_id));
    ...
    const handleSaveVariant = async () => {
        ...
        const { price, quantity, sku, color_name, size_name, sale_price, sale_start, sale_end } = editingVariant;

        // Kiểm tra giá trị trước khi gửi yêu cầu
        if (isNaN(price) || isNaN(quantity)) {
            toast.error("Giá và số lượng phải là số hợp lệ.");
            return;
        }

        // Kiểm tra nếu sale_price chưa có giá trị
        if (sale_price === undefined || isNaN(sale_price)) {
            toast.error("Giá bán (sale_price) là bắt buộc và phải là số hợp lệ.");
            return;
        }

        // Kiểm tra nếu sale_start chưa có giá trị
        if (!sale_start) {
            toast.error("Ngày bắt đầu chương trình khuyến mãi là bắt buộc.");
            return;
        }

        // Kiểm tra nếu sale_end chưa có giá trị
        if (!sale_end) {
            toast.error("Ngày kết thúc chương trình khuyến mãi là bắt buộc.");
            return;
        }
        ...
        try {
            const response = await axios.put(`http://127.0.0.1:8000/api/product-variants/${editingVariant.id}`, {
    ...
    if (!product) return <div className="text-center text-lg">Không tìm thấy sản phẩm.</div>;

    return (
        <div className="max-w-6xl mx-auto p-6 bg-gray-100 shadow-lg rounded-lg mt-10">
            <h1 className="text-4xl font-bold text-gray-800 text-center mb-6">{product.name}</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                    <p className="text-lg font-semibold">
                        <span className="text-gray-600">Danh mục:</span> {category ? category.name : "Không xác định"}
                    </p>
                    <p className="text-lg font-semibold">
                        <span className="text-gray-600">Trạng thái:</span>{" "}
                        <span className={`${product.status === 1 ? "text-red-500" : "text-green-500"} font-bold`}>
                    ...
                    {variants.length > 0 && (
                        <div>
                            <h2 className="text-2xl font-bold text-gray-800 mb-4">Biến thể sản phẩm:</h2>
                            <div className="space-y-4">
                                    ...
                                    return (
                                        ...
                                        >
                                            <p className="text-gray-700">
                                                <strong>Màu sắc:</strong> {color ? color.name : "Không xác định"}
                                            </p>
                                            <p className="text-gray-700">
                                                <strong>Kích thước:</strong> {size ? size.name : "Không xác định"}
                                            </p>
                                            <p className="text-gray-700">
            ...
            {editingVariant && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                    <div className="bg-white p-6 rounded-lg shadow-lg w-1/2">
                        ...
                        <div className="mb-4">
                            <label className="block text-gray-700">Ngày bắt đầu khuyến mãi:</label>
                            <input
                        ...
                        <div className="mb-4">
                            <label className="block text-gray-700">Ngày kết thúc khuyến mãi:</label>
                            <input
                                type="date" // Sử dụng type date thay vì datetime-local
