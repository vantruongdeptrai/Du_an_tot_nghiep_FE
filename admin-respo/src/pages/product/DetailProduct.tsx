...
const DetailProduct: React.FC = () => {
  ...
  const handleSaveVariant = async () => {
    ...
    if (!size || !color) {
      toast.error("Kích thước hoặc màu sắc không hợp lệ.");
      return;
    }
  ...
  const checkIfCombinationExists = (sizeId: number, colorId: number) => {
    return variants.some(
      (variant) => variant.size_id === sizeId && variant.color_id === colorId
    );
  };
  return (
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-1/2">
            ...
            <div className="mb-4">
              ...
              <input
                type="datetime-local" // Sử dụng type date thay vì datetime-local
                value={
                  editingVariant.sale_start
                    ? new Date(editingVariant.sale_start)
                        .toISOString()
                        .slice(0, 16)
                    : ""
                }
            ...
            <div className="mb-4">
              ...
              <input
                type="datetime-local" // Sử dụng type date thay vì datetime-local
                value={
                  editingVariant.sale_end
                    ? new Date(editingVariant.sale_end)
                        .toISOString()
                        .slice(0, 16)
                    : ""
                }
            ...
            <div className="mb-4">
              ...
              <select
                value={editingVariant.color_name || ""} // Đảm bảo giá trị color_name được thiết lập
                onChange={(e) => {
                  setEditingVariant({
                    ...editingVariant,
              ...
              >
                {colors.map((color) => {
                  // Disable những màu sắc đã có với size hiện tại
                  const isColorDisabled = checkIfCombinationExists(
                    sizes.find((s) => s.name === editingVariant.size_name)
                      ?.id ?? -1,
                    color.id
                  );
                  return (
                    ...
                    >
                      {color.name} {isColorDisabled && "(Đã chọn với size này)"}
                    </option>
                  );
                })}
              </select>
            ...
            <div className="mb-4">
              ...
              <select
                value={editingVariant.size_name || ""} // Đảm bảo giá trị size_name được thiết lập
                onChange={(e) => {
                  setEditingVariant({
                    ...editingVariant,
              ...
              >
                {sizes.map((size) => {
                  const isSizeDisabled = checkIfCombinationExists(
                    size.id,
                    colors.find((c) => c.name === editingVariant.color_name)
                      ?.id ?? -1
                  );
                  return (
                    <option
                    ...
                    >
                      {size.name} {isSizeDisabled && "(Đã chọn với màu này)"}
                    </option>
                  );
                })}
              </select>
