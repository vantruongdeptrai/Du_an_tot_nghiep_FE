...
interface StatsProps {
  ...
  revenueByCategory:
    | Array<{ category_name: string; revenue: number }>
    | undefined;
...
const Stats = ({
  ...
  const totalBestSellers =
    bestSellers.success && Array.isArray(bestSellers.data)
      ? bestSellers.data.reduce(
          (total, item) => total + parseInt(item.total_sold || "0"),
          0
        )
      : 0;

  // Tính tổng doanh thu theo danh mục
  const totalRevenueByCategory = revenueByCategory.reduce(
  ...
  return (
    <div>
      <h2 className="text-3xl text-whiteSecondary font-bold mb-7">
        Balance Overview
      </h2>
